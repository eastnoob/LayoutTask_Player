...
- Both the tutorial reference board and interactive tutorial must resolve through the tutorial package base URL.
...
### Task 3: Route both tutorial flows through the fixed package
...
- Produces: timeline entries where reference-board media and the interactive tutorial both use the resolved tutorial base URL, while formal entries use `config.baseUrl`.

- [ ] **Step 1: Write failing timeline URL tests.**

Set `tutorial.baseUrl` to `/layout-task-tutorial/` and assert that every reference-board asset URL contains `/layout-task-tutorial/`, the tutorial `LayoutTaskPlugin` uses that base, and the first formal trial uses `/layout-task-run12-core23-preview/`.
...
Expected: the existing reference-board assertion fails because the runner currently passes `config.baseUrl` to the board builder.

- [ ] **Step 3: Implement the base URL routing.**

Change only the reference-board call to use:
...
Keep formal trial `baseUrl: config.baseUrl` unchanged and keep the interactive tutorial fallback for older configs.
...
Verify a config without `tutorial.baseUrl` still works, a config with it never routes formal trials to the tutorial package, and reference-board HTML has no formal package path.
...
### Task 6: Protect the tutorial package from compiler output
...
**Interfaces:**
- Consumes: formal compiler output root and fixed tutorial package root.
- Produces: formal compilation behavior unchanged, with a verification that the tutorial package is untouched.

- [ ] **Step 1: Write a failing protection test.**

Create a temporary formal output root and a temporary tutorial package snapshot. Run `compileBatchToDirectory` from `tools/generator/compile-batch.ts` with the formal root and assert the tutorial snapshot is byte-for-byte unchanged. Also assert the compiler rejects `public/layout-task-tutorial/` as its output root before writing.
...
Make the formal compiler reject `public/layout-task-tutorial/` as an output root and keep all generated files under the requested formal package root. Do not add a destructive cleanup step that can traverse the parent `public/` directory.
...
Test both a normal formal compile and an attempted tutorial-root compile. Confirm the former succeeds without changes to the tutorial package and the latter fails before writing.

- [ ] **Step 6: Commit.**

```powershell
git add tools/generator/compile-batch.ts tools/generator/compile-batch.test.ts
git commit -m "guard: protect fixed tutorial package from compilation"
```
...
Build the timeline and assert tutorial/reference-board URLs use the fixed package while formal tasks use the formal package.
...
Expected: all tests pass, build succeeds, and `dist/layout-task-tutorial/` plus the formal package are present in the static output.
...
Verify the reference board loads, the interactive tutorial loads, and no request points to a newly generated tutorial package. Then verify a formal entry still resolves against `layout-task-run12-core23-preview`.
...
- Fixed package and base routing: Tasks 2-3.
...
- Build/static/browser verification: Task 7.
...
Task 1 produces `ExperimentTrialResultItem.trialType` and `collectTutorialTrialResult`; Task 4 consumes both. Task 2 produces the package version/lock identity; Tasks 4 and 7 consume it. Task 4 produces the complete `ExperimentCsvFile[]`; Task 5 consumes it. Task 6 is independent of the data interfaces and only protects the package root.
